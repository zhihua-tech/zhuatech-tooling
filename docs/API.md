...
- `retire`：停用报废；READY / MAINTENANCE → RETIRED；最低角色 REVIEWER；禁止创建者自审。业务输入：无，但必须填写 remark。

### 工装领用 `assignments`

- `issue`：领用出库；DRAFT → IN_USE；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `return`：归还入库；IN_USE → RETURNED；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `cancel`：撤销；DRAFT → CANCELLED；最低角色 OPERATOR。业务输入：无，但必须填写 remark。

### 生产计次 `runs`

- `post`：确认计次；DRAFT → POSTED；最低角色 OPERATOR。业务输入：无，但必须填写 remark。

### 保养维修 `maintenance`

- `start`：开始保养；DRAFT → IN_PROGRESS；最低角色 OPERATOR。业务输入：无，但必须填写 remark。
- `complete`：提交完工；IN_PROGRESS → REVIEW；最低角色 OPERATOR。业务输入：work（完成工作）、cost（维修成本）。
- `approve`：验收解锁；REVIEW → CLOSED；最低角色 REVIEWER；禁止创建者自审。业务输入：无，但必须填写 remark。
